...
bool WriteMesh(Vertex* vertices, unsigned int width, unsigned int height, const std::string& filename)
{
	...
	// TODO: save vertices
        for(int i = 0; i<nVertices;i++)
        {
           ...
           outFile << vertices[i].position[0] << " " << vertices[i].position[1] << " " << vertices[i].position[2] << " " << (int)vertices[i].color[0] << " " << (int)vertices[i].color[1] << " " << (int)vertices[i].color[2] <<std::endl;
        }
...
int main()
{
	...
	while (sensor.ProcessNextFrame())
	{
		...
		Vertex* vertices = new Vertex[sensor.GetDepthImageWidth() * sensor.GetDepthImageHeight()];
                ...
                for(int i=0; i<sensor.GetDepthImageWidth() * sensor.GetDepthImageHeight() ; i++)
                {
                    if(depthMap[i] == MINF)
                    {
                        vertices[i].position = Vector4f(0, 0, 0, 0);
                    ...
                    else
                    {
                        ...
                        //Vector3f pixel = Vector3f(float(rowMatrix), float(colMatrix), 1);
                        //Vector3f camera = depthIntrinsicsInv*pixel*depthMap[i];
                    ...
                    //vertices[i].color = Vector4uc(255,255,255,255);
                }
